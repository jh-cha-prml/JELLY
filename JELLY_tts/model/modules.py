...
class VarianceAdaptor(nn.Module):
    ...
    def forward(
        ...
        if intensity_embedding is not None:
            # print(f'intensity_embedding.shape : {intensity_embedding.shape}')
            x = x + intensity_embedding.unsqueeze(1).expand(
                -1, text.shape[1], -1
            )
            
        # intensity 다음으로 emotion 오도록 순서 바꿈
        if emotion_embedding is not None:
            # print(f'emotion_embedding.shape : {emotion_embedding.shape}')
