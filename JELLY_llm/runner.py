...
class Runner:
    ...
    def save_result(self, result, result_dir, filename):
        ...
        if is_main_process():
            ...
            #### txt for stage03 inference ####
            json_path = final_result_file
            output_file = os.path.join(result_dir, 'test_frame_pred.txt')
            infer_txt_path ="../JELLY_tts/preprocessed_data/DailyTalk/test_frame_pred.txt"
            ...
            id_nested_list = df['id'].tolist()
            ground_truth_nested_list = df['ground_truth'].tolist()
            text_nested_list = df['text'].tolist()

            id_list = [item.split('/')[-1].replace('.wav', '') for sublist in id_nested_list for item in sublist] # path
            ground_truth_list = [item for sublist in ground_truth_nested_list for item in sublist] # GT
            text_list = [item for sublist in text_nested_list for item in sublist] # prediction

            combined_lines = [f"{id}|{ground_truth}|{text}" for id, ground_truth, text in zip(id_list, ground_truth_list, text_list)]

            with open(output_file, 'w') as f:
                f.write('\n'.join(combined_lines))
                
            intensity_list = ["weak", "medium", "strong"]
            emotion_list = ["disgust", "neutral", "surprise", "happy", "fear", "sad", "angry"]

            
            intensity_dict = {"weak": 1, "medium": 2, "strong": 3}
            emotion_int_dict = {"disgust": 0, "none": 1, "surprise": 2, "happiness": 3, "fear": 4, "sadness": 5, "anger": 6}
            emotion_dict = {"disgust": "disgust", "neutral": "none", "surprise": "surprise", "happy": "happiness", "fear": "fear", "sad": "sadness", "angry": "anger"}
            
            test_pred_emo = []
            test_pred_intensity = []

            wrong_pred_emo_list = []
            wrong_pred_intensity_list = []
            
            total_count = 0
            wrong_count = 0
            for id, gt, prediction in zip(id_list, ground_truth_list, text_list) :
                pred_intensity = ""
                pred_emo = ""
                ...
                if "neutral" in prediction :
                    ...
                    for target_intensity in intensity_list :
                        if target_intensity in prediction :
                            pred_intensity += target_intensity
                            
                    for target_emo in emotion_list :
                        if target_emo in prediction :
                            pred_emo += target_emo
                
                if pred_intensity in intensity_dict.keys():
                    pred_intensity = intensity_dict[pred_intensity]
                else :
                    pred_intensity = 1
                    wrong_pred_intensity_list.append(pred_intensity)
                    print("out of intensity category answer")
                    print()
                test_pred_intensity.append(pred_intensity)
                
                    
                if pred_emo in emotion_dict.keys():
                    pred_emo = emotion_dict[pred_emo]
                else :
                    pred_emo = "none"
                    wrong_pred_emo_list.append(pred_emo)
                    print("out of emotion category answer")
                test_pred_emo.append(pred_emo)
                
            combined_lines = [f"{id}|{pred_emo}|{pred_intensity}" for id, pred_emo, pred_intensity in zip(id_list, test_pred_emo, test_pred_intensity)]

            with open(output_file, 'w') as f:
                f.write('\n'.join(combined_lines))

            print("result txt saved to %s" % output_file)
            
            data = [
                {
                    id: {
                        "emotion": emotion_int_dict[pred_emo],
                        "intensity": int(pred_intensity - 1)
                    }
                }
                for id, pred_emo, pred_intensity in zip(id_list, test_pred_emo, test_pred_intensity)
            ]
            ...
            with open(output_file, "r") as file:
                for line in file:
                    ...
                    filename, emotion, intensity = parts[0], parts[1], parts[2]
                    jelly_pred_result[filename] = (emotion, intensity)

            with open("../JELLY_tts/preprocessed_data/DailyTalk/test_frame.txt", "r") as file:
            ...
            for line in lines:
                parts = line.strip().split('|')
                filename = parts[0]
                if filename in jelly_pred_result:
                    # Update the emotion and intensity
                    emotion, intensity = jelly_pred_result[filename]
                    parts[-2] = emotion
                    parts[-1] = intensity
                updated_lines.append('|'.join(parts))

            with open(infer_txt_path, "w") as file:
                file.write('\n'.join(updated_lines))

            print(f"File updated successfully: {infer_txt_path}")
