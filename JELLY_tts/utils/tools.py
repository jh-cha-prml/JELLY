...
def to_device(data, device):
    if len(data) == 17: #16
        ...
        if intensitys is not None:
            intensitys = torch.from_numpy(intensitys).long().to(device)
    ...
    if len(data) == 10: #9
        (ids, raw_texts, speakers, texts, src_lens, max_src_len, spker_embeds, emotions, intensitys, history_info) = data
        ...
        return (ids, raw_texts, speakers, texts, src_lens, max_src_len, spker_embeds, emotions, intensitys, history_info)
