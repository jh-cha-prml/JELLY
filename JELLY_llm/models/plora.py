...
class LoraModel(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def __init__(self, model, config_list, adapter_name_list):
        super().__init__()
        self.model = model
        ...
        for config, adapter_name in zip(config_list, adapter_name_list): 
            self.add_adapter(adapter_name, config) # token마다 다르게 적용할 adapter 추가
    ...
    def add_adapter(self, adapter_name, config=None):
        ...
        #     ) #hyeokiki #나는 토큰 종류에 따라서 하나의 adapter만 적용하도록 할거라 괜찮을 듯
...
class Embedding(nn.Embedding, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.active_adapters =  {adapter_name}#hyeokiki
    ...
    def merge(self):
        ...
        # hyeokiki #
        for active_adapter in self.active_adapters:
            if active_adapter in self.lora_embedding_A.keys() and self.r[active_adapter] > 0:
                self.weight.data += (
                    transpose(
                        self.lora_embedding_B[active_adapter] @ self.lora_embedding_A[active_adapter], True
                    )
                    * self.scaling[active_adapter]
                )
    ...
    def forward(self, x: torch.Tensor):
        #hyeokiki
        if all(adapter not in self.lora_embedding_A.keys() for adapter in self.active_adapters):
            return nn.Embedding.forward(self, x)
        #/hyeokiki
        
        if self.disable_adapters:
            #hyeokiki
            if any(self.r[adapter] > 0 for adapter in self.active_adapters) and self.merged:
                self.unmerge()
            #/hyeokiki
        ...
        #hyeokiki
            ...
            for adapter in self.active_adapters:
                ...
                after_A = F.embedding(
                    x,
                    self.lora_embedding_A[adapter].T,
                    self.padding_idx,
                    self.max_norm,
                    self.norm_type,
                ...
                output = (after_A @ self.lora_embedding_B[adapter].T) * self.scaling[adapter]
                # lora_mask = self.lora_mask.get(adapter, None)
                ...
                #         if self.expect_inference_prompt:
                #             result += output * lora_mask.unsqueeze(-1)
                #             self.expect_inference_prompt = False  # 프롬프트 처리 완료
                #     else:
                #         if not lora_mask.size() == output.size()[:-1]:
                #             raise ValueError(f"The dimension of lora_mask {lora_mask.size()} does not match the "
                #                             f"feature tensor {output.size()}")
