...
class Dataset(Dataset):
    def __init__(
        ...
        self.load_intensity = model_config["multi_intensity"]
        ...
        self.basename, self.speaker, self.text, self.raw_text, self.emotion, self.intensity = self.process_meta(
            filename
        )
        ...
        with open(os.path.join(self.preprocessed_path, "score.json")) as f:
            self.intensity_map = json.load(f)
        self.sort = sort
    ...
    def __getitem__(self, idx):
        ...
        intensity_id = self.intensity_map[self.intensity[idx]] if self.load_intensity else None
    ...
    def process_meta(self, filename):
        with open(
            ...
            for line in f.readlines():
                ...
                if self.load_emotion and self.load_intensity:
                    n, s, t, r, e, i = line.strip("\n").split("|")
                elif self.load_emotion and not self.load_intensity:
            ...
            return name, speaker, text, raw_text, emotion, intensity
    ...
    def reprocess(self, data, idxs):
        ...
        intensitys = np.array([data[idx]["intensity"] for idx in idxs]) if self.load_emotion else None
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (basename, speaker_id, phone, raw_text, spker_embed, emotion_id, intensity_id, history)
    ...
    def collate_fn(self, data):
        ...
        intensitys = np.array([d[6] for d in data]) if self.load_intensity else None
        ...
        return ids, raw_texts, speakers, texts, text_lens, max(text_lens), spker_embeds, emotions, intensitys, history_info
